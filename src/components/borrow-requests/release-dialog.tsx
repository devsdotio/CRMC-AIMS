...
export interface ReleaseDialogProps {
  ...
  onConfirm: (
    ...
    payload: { pickedUpBy: string; note?: string }
  ) => Promise<void>;
}
...
export function ReleaseDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onConfirm(request, { pickedUpBy: pickedUpBy.trim(), note: note.trim() || undefined });
      setPickedUpBy("");
      setNote("");
      onClose();
    } catch (err) {
      console.error(err);
      setError("Failed to release item. Please try again.");
    } finally {
        ...
        <form onSubmit={handleSubmit} className="flex flex-col">
          <div className="p-6 flex flex-col gap-5">
            <div className="space-y-2">
              <label htmlFor="pickedUpBy" className="text-sm font-semibold text-text">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={cn(
                    ...
                    error && !pickedUpBy.trim() && "border-status-outofservice-bg focus:ring-status-outofservice-bg"
                  )}
                  value={pickedUpBy}
            ...
            <div className="space-y-2">
              ...
              <textarea
                id="note"
                placeholder="Any comments about the release condition..."
                disabled={isSubmitting}
                className={cn(
          ...
          <div className="px-6 py-4 bg-bg-subtle/50 border-t border-border flex items-center justify-end gap-3">
            <button
              ...
              disabled={isSubmitting}
              onClick={onClose}
              className="px-4 py-2 text-sm font-semibold text-text-secondary hover:text-text hover:bg-bg-subtle rounded-md transition-colors disabled:opacity-50"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={isSubmitting}
              className="inline-flex items-center justify-center min-w-30 px-4 py-2 bg-accent text-accent-foreground text-sm font-semibold rounded-md shadow-xs hover:opacity-90 transition-opacity disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {isSubmitting ? (
                <>
